...
class Basic_Animations(Scene):
    ...
    def color_point(self, p, color):
        point = Dot(point=np.array([p.x, p.y, 0]))
        point.set_color(color)
        self.add(point)
        self.wait(0.5)

    def color_point_tupla(self, tupla, color):
        x, y = tupla
        point = Dot(point=np.array([x, y, 0]))
        point.set_color(color)
        self.add(point)
        self.wait(0.5)
    ...
    def color_edge(self, edge, color):
        p1 = edge.p1
        p2 = edge.p2
        self.color_line_from_points(p1, p2, color)
    ...
    def color_triangle(self, p1, p2, p3, color):
        self.color_line_from_points(p1, p2, color)
        self.color_line_from_points(p2, p3, color)
        self.color_line_from_points(p3, p1, color)
